...
export async function GET(req: NextRequest) {
  try {
    ...
    const transformedData: IndividualRegistrationWithRound[] = (data as Recruitment25Data[]).map(item => {
      // Determine which round to show based on which domain is "creatives"
      let displayRound: number;
      if (item.domain1.toLowerCase().includes('creatives')) {
        displayRound = item.domain1_round;
      } else if (item.domain2 && item.domain2.toLowerCase().includes('creatives')) {
