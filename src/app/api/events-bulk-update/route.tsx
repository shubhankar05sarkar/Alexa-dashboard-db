...
export async function POST(req: NextRequest) {
  try {
    ...
    // Update domain1_round for records where domain1 is events
    const eventsDomain1Records = currentRecords?.filter(record => 
      record.domain1.toLowerCase().includes('events')
    ) || [];

    if (eventsDomain1Records.length > 0) {
      const { error: domain1Error } = await userSupabase
        ...
        .in('registration_number', eventsDomain1Records.map(r => r.registration_number));
    ...
    // Update domain2_round for records where domain2 is events
    const eventsDomain2Records = currentRecords?.filter(record => 
      record.domain2 && record.domain2.toLowerCase().includes('events')
    ) || [];

    if (eventsDomain2Records.length > 0) {
      const { error: domain2Error } = await userSupabase
        ...
        .in('registration_number', eventsDomain2Records.map(r => r.registration_number));
