...
export async function POST(req: NextRequest) {
  try {
    ...
    const technicalDomain1Records = currentRecords?.filter(record => 
    ...
    if (technicalDomain1Records.length > 0) {
      const { error: domain1Error } = await userSupabase
        ...
        .in('registration_number', technicalDomain1Records.map(r => r.registration_number));
    ...
    const technicalDomain2Records = currentRecords?.filter(record => 
    ...
    if (technicalDomain2Records.length > 0) {
      const { error: domain2Error } = await userSupabase
        ...
        .in('registration_number', technicalDomain2Records.map(r => r.registration_number));
