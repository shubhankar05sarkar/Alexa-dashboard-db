...
export default function IndividualRegistrationTable({ registrations }: { registrations: IndividualRegistration[] }) {
  return (
    <div className="overflow-x-auto">
      ...
      <table className="min-w-full divide-y divide-gray-700 hidden md:table">
        <thead className="bg-gray-800">
          <tr>
            ...
            <th className="px-6 py-3 text-left text-xs font-medium text-purple-300 uppercase tracking-wider">Reg No</th>
        ...
        <tbody className="bg-gray-900/50 divide-y divide-gray-700">
          {registrations.map((reg) => (
            <tr key={reg.id} className="hover:bg-gray-800/50 transition-colors">
              ...
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300">{reg.registerNumber}</td>
      ...
      <div className="flex flex-col gap-4 md:hidden">
        {registrations.map((reg) => (
          <div key={reg.id} className="bg-gray-900/50 border border-gray-700 rounded-lg p-4">
            <p className="font-bold text-white text-lg mb-1">{reg.name}</p>
            <p className="text-gray-300"><span className="font-semibold text-purple-200">Reg No:</span> {reg.registerNumber}</p>
