...
import {
  IndividualRegistrationWithRound,
  RecruitmentData,
} from "../../types/types";
...
export default function CreativesPage() {
  ...
  const handleExport = () => {
    ...
    setToastMessage(
      `${filteredRegistrations.length} participants exported successfully`,
    );
    setTimeout(() => setToastMessage(""), 3000);
  };
  ...
  const handleBulkUpdate = () => {
    ...
    Papa.parse(bulkFile, {
      ...
      complete: async (results: ParseResult<CSVRow>) => {
        ...
        if (
          dataRows.length > 0 &&
          !dataRows[0].hasOwnProperty("registerNumber")
        ) {
          setToastMessage(
            "CSV file must have a column named 'registerNumber'. Please check and try again.",
          );
          setTimeout(() => setToastMessage(""), 5000);
          return;
        }

        const regNumbers: string[] = dataRows.map(
          (row) => row.registerNumber?.trim() || "",
        );
        ...
        try {
          ...
          const updatedRegNumbers = data.map(
            (record: DatabaseRecord) => record.registration_number,
          );
          const missingRegNumbers = regNumbers.filter(
            (regNum) => !updatedRegNumbers.includes(regNum),
          );
          ...
          let successMessage =
            data.message ||
            `Successfully updated ${data.length} participants to round ${bulkRound}`;
          if (missingRegNumbers.length > 0) {
            const missingNumbers = missingRegNumbers
              .map((num) => `registration number: ${num}`)
              .join(", ");
          ...
          <div className="bg-white/10 backdrop-blur-md rounded-xl shadow-lg overflow-hidden max-w-6xl mx-auto border border-white/20">
            ...
            <div className="p-6">
              ...
              <div className="border border-white/20 rounded-lg overflow-hidden bg-gray-900/50 backdrop-blur-sm">
                <IndividualRegistrationTableWithRound
